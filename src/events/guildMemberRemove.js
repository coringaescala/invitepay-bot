  ...
  async execute(member, client) {
    ...
    if (!referral) {
      await sendAdminLog(client, {
        content: `📤 ${member.user.tag} saiu do servidor sem referral registrado.`
      });
      return;
    }

    const reward = Number(referral.rewardAmount || 1);
    ...
    await sendPublicLog(client, {
      content: `📤 ${member.user.username} saiu do servidor.`
    });
...
👥 Referências atuais: ${updatedUser.totalReferrals}`
    ...
    console.log(`${referral.referrer.username} perdeu R$${reward}`);
  }
};